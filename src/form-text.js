...
export default function FormText(){
    ...
    const onBlurPassword = () =>{
        ...
        if(pswd !== '' && !pswd.match(/^[0-9a-zA-Z]+$/)){
            textPswd.current.valus =''
            alert('ต้องเป็น 0-9 หรือ a-z หรือ A-Z')
        }
    }
    const onBlurPassword2 = () =>{
        if(textPswd.current.valus !== textPswd2.current.valus){
            textPswd2.current.valus = ''
        ...
        <div className="mt-4 mx-auto p-3 rounded" style={{width:'400px', background:'#cee'}}>
            <form>
                ...
                <div className="text-center">
                    <button className="btn btn-sm px-4" style={{backgroundColor:'skyblue'}}>OK</button>
                </div>
            </form>
